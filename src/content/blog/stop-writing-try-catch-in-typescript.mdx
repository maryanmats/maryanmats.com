...
**Update:** I packaged these patterns into [`ts-safe-result`](https://www.npmjs.com/package/ts-safe-result) — a tiny (< 1KB), zero-dependency `Result<Value, Error>` type for TypeScript with `ok`, `err`, `map`, `flatMap`, `match`, `tryCatch`, `tryAsync`, and more. If you want to start using typed errors in your codebase today, it's ready to go.
