...
**Update:** I packaged the `as const` + union type pattern into [`ts-safe-enum`](https://www.npmjs.com/package/ts-safe-enum) — a 536-byte `defineEnum()` function with full type inference, type guards, validation, and reverse lookup. No `as const` needed, no runtime surprises.
